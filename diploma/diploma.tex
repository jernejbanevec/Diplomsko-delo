...
\subsection{Optimizacija optimizacijskega kriterija $J_1 = sled({S^\ell_2}^{-1} S^\ell_1)$ za nesingularno matriko $S_2$}
...
\text{,}
$$
kjer je $\Lambda = diag\left(\lambda_1, \ldots, \lambda_m \right)$ diagonalna matrika. S preoblikovanjem zgornje enačbe pa lahko dobimo
$$
S_1 = VY \Lambda Y^TV^T = X \Lambda X^T
\text{,}
$$
kjer smo dodatno definirali $X := VY$.
Matriko $S_2$ pa preoblikujemo
$$
...
\end{split}
\end{equation}
kjer je matrika $\tilde{G} := X^{-1} G \in \mathbb{R}^{m \times \ell}$. Ker vemo, da ima matrik $X$ poln rang, ima matrika $\tilde{G}$ rang enak številu stolpec, torej $\ell$ in tako lahko na njen naredimo $QR$ razcep in tako dobimo matriko $Q \in \mathbb{R}^{m \times \ell}$, ki ima ortonormirane stolpce in nesingularno matriko $R \in \mathbb{R}^{\ell \times \ell}$, da velja $\tilde{G} = QR$. Tako lahko zgornjo enačbo $\textit{\eqref{pospJ1}}$ dodatno preoblikujemo
\begin{align*}
J_1(G)
=
...
\text{,}
\end{align*}
ker pa vemo, da imata podobni matriki enako sled, lahko to matriko z leve množimo z $R$, z desne pa z $R^{-1}$ in tako dobimo
\begin{align*}
J_1(G)
=
...
\text{.}
\end{align*}
Za vsak $\ell \geq q$, ima optimizacijski kriterij doseže svojo zgornjo mejo za 
$$ Q = 
\begin{bmatrix}
I_{\ell} \\
0_{m-\ell, \ell}
...
\text{,}
\end{align*}
ker pa velja, da imata podobni matriki enako sled, lahko zgornjo enačbo iz leve množimo z $W$ in iz desne z $W^{-1}$ in tako dobimo
\begin{align*}
...
\text{.}
\end{equation}
V naslednjem poglavju maksimizacijo maksimizacijskega kriterija $J_1(G)$ posplošimo tudi na primer, ko je matrika $S_2$ singularna.

\subsection{Posplošitev maksimizacijskega kriterija $sled((S^\ell_W)^{-1}S^\ell_B)$ za singularno matriko $S_2$}
...
\end{bmatrix}
$, kjer je
$r = rang(K) - rang(K_B)$,  $s = rang(K_A) + rang(K_B) - rang(K)$ ter $k = rang(K)$. Za elemente matrik $D_A$ in $D_B$ pa velja
...
X^{-1} \text{,}
\end{align*}
kjer za matrika $\Sigma_A^T \Sigma_A \in \mathbb{R}^{k \times k}$ enaka diagonalni matriki $\text{diag}\left(\alpha_1^2, \ldots, \alpha_t^2 \right)$. V kolikor tu gledamo le $i$-ti stolpec ortogonalne matrike $X$, ki ga označimo z $x_i$ ter dodatno definiramo $\alpha_i = 0$ za $i = k+1, \ldots, m$, za $i = 1, 2, \ldots, m$ dobimo
$$
x_i^T H_B H_B^T x_i = \alpha_i^2
\text{,}
$$
če pa to enačbo množimo z desne z $x_i$ dobimo
...
\text{.}
\end{equation}
Tu smo dodatno definirali $\beta_i = 0$ za $i = k+1, \ldots, m$.
...
Najprej si bomo pogledali primer rešitve tega problema za singularno matriko $H_W$, nato pa bomo potegnili vzporednice med to metodo in metodo iz prejšnjega poglavja, ki velja le za nesingularno matriko $S_W$.
...
\text{,}
$$
kjer $g_j$ predstavlja $j$-ti stolpec preslikave $G$. Ke velja $x_i^T S_W x_i = 0$ in $x_i^T S_B x_i = 0$, dodajanje teh stolpcev v preslikavo $G$ ne bo imelo vpliva niti na maksimizacijo niti na minimizacijo iz \textit{\eqref{pogoji-G}}. Posledično teh stolpcev $x_i$ ne vključimo v $G$.
...
\end{enumerate}
Tako lahko zaključimo, da tudi ko je matrika $S_W$ singularna, preslikavo $G \in \mathbb{R}^{m \times \ell}$ sestavimo iz $\ell$ levih stolpcev matrike $X$.



\subsubsection{Matrika $H_W$ polnega ranga}
V tem primeru bo zagotovo veljalo $n > m$. Matika $H_W$ bo v tem primeru imela poln stolpični rang, torej $\text{rang}(H_W) = m$, iz česar pa sledi $r = m - m = 0$, $s = \text{rang}(H_B^T) + m - m = \text{rang}(H_B^T)$ ter $k = m$. Od tod sledi $\beta_i \neq 0 \hspace{1mm} \forall \hspace{1mm} i = 1, \ldots, m \text{.}$ Posledično lahko v enačbi \textit{\eqref{alpha-beta-1}} delimo z $\beta_i$ in dobimo 
$$
\frac{\alpha_i^2}{\beta_i^2} H_W H_W^T x_i =  H_B H_B^T x_i
\text{.}
$$
Ker pa nam posplošeni singularni razcep vrne pare lastnih singularnih vrednosti $(\alpha_i, \beta_i)$ urejene v sledečem vrstnem redu $1 > \alpha_{r+1} \geq \alpha_{r+s} > 0$ in $0 < \beta_{r+1} \leq \beta_{r+s} < 1$, velja, da so koeficienti $\lambda_i = \frac{\alpha_i^2}{\beta_i^2}$ v nenaraščajočem vrstnem redu. Ker za poljuben $i \in {1, \ldots, s}$ velja
$$
...
\text{,}
$$
so koeficienti $\frac{\alpha_i}{\beta_i}$ nenaraščajoči, dodatno pa iz $\alpha_i \geq 0$ in $\beta_i \geq 0$ za $i = 1, \ldots, m$ sledi, da so tudi koeficienti $\frac{\alpha_i^2}{\beta_i^2}$ v nenaraščajočem vrstnem redu. Sledi, da potrebujemo le prvih $k - 1$ stolpcev matrike $X$.
